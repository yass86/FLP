...
class Slide extends CI_Controller {
    ...
    function bloquenuevo($seccion = "") {
        ...
        if ($seccion != "") {
            ...
            $dat['seccion'] = $seccion;            
            $dat['titulo'] = "";          
            $pagina = $this->load->view('admin/slide/nuevo_slide', $dat, true);
        }
        echo $pagina;
    }
    ...
    function registrar_slide() {
        ...
        if ($this->seccionActiva()) {
            ...
            $inserto = $this->slide->nuevoslide($var);
            if ($inserto)
                redirect('admin');
            else
    ...
    function nuevo_contenido_slide() {

        if ($this->seccionActiva()) {
            $var = array();
            $var['id'] = $this->input->post('id_img', true);
            $var['id_slide'] = $this->input->post('id_slide', true);
            $var['titulo'] = $this->input->post('titulo', true);
            $var['contenido'] = $this->input->post('contenido', true);
            $var['imagen'] = $this->almacenarFoto();
            $var['orden'] = $this->input->post('orden', true);
            $var['txtboton'] = $this->input->post('txtboton', true);
            $var['urlboton'] = $this->input->post('urlboton', true);
            
           
            $this->load->model('slide/slide_model', 'slide');
            $inserto = $this->slide->nuevo_contenido_slide($var);
            if ($inserto)
                redirect('admin/ir/nuevo-cotenido-slide');
            else
                redirect('admin/error');
        }
        else {
